...
ZWROTY_FILE = "Zwroty"
ETYKIETY_FILE = "Etykiety" # Nowa zakładka na kawałki plików PDF
...
SHEET_HEADERS = {
    ...
    "Etykiety": ["zam_id", "czesc", "dane"] # Nagłówki dla bazy PDF
}
...
def load_data(sheet_name):
    try:
        ...
        df = df.fillna("") # Zabezpieczenie przed błędami 'NaN'
...
# Funkcja usuwająca etykietę po zakończeniu zamówienia
...
def move_to_history(order_id):
    ...
    if order:
        ...
        usun_etykiete(order_id) # Zwalniamy miejsce w arkuszu etykiet
...
def restore_from_history(order_id):
    ...
    if order:
        ...
        order['ma_etykiete'] = "False" # Cofnięte nie ma już etykiety
        zam.append(order)
                    ...
                    if st.form_submit_button("PRZEKAŻ NA MAGAZYN", type="primary"):
                        if nr and co:
                            if len(zam_data) > 0 and str(zam_data[-1].get('nr')) == str(nr):
                                ...
                                # CIĘCIE PLIKU I ZAPIS DO ARKUSZA "Etykiety"
                ...
                for z in zam_data:
                    with st.expander(f"ZAM: {z['nr']}  |  Wymagany termin: {z.get('termin', 'Brak')}"):
                        ...
                        if str(z.get('ma_etykiete')).lower() == 'true': 
                            info_text += "<br><span style='color:#1e3a8a;'>📄 Dołączono etykietę PDF w chmurze</span>"
                        ...
                        if col_action.button("Wycofaj (Usuń)", key=f"boss_cancel_{z['id']}", use_container_width=True):
                            ...
                            save_data(ZAM_FILE, zam_data)
                            usun_etykiete(z['id']) # Usuwa etykietę powiązaną ze zleceniem
                            st.rerun()
    ...
    # ==========================================
        ...
        with tab_kds:
            if not zam_pracownik:
                ...
                # Ładujemy bazę etykiet na zewnątrz pętli, żeby było błyskawicznie
                ...
                for i, z in enumerate(zam_pracownik):
                    with cols[i % 3]:
                        with st.container(border=True):
                            ...
                            # --- PRZYCISK DRUKOWANIA W HTML (BEZ RERUNU!) ---
                            if str(z.get('ma_etykiete')).lower() == 'true':
                                # Sklejanie z kawałków z Arkusza 'Etykiety'
                                ...
                                if kawalki:
                                    ...
                                    """
                                    # Dodanie ukrytego widgetu HTML z przyciskiem, który po kliknięciu wywołuje okno drukarki, a Streamlit "nie widzi" kliknięcia, więc nie ma rerunu!
                ...
                for h in hist:
                    with st.expander(f"✔️ ZAM: {h.get('nr')}  |  Wykonano: {h.get('data_pakowania', '')}"):
                        ...
                        if col2.button("Cofnij zlecenie na ekran", key=f"w_undo_{h['id']}", use_container_width=True):
                            restore_from_history(h['id'])
                            st.rerun()        border-radius: 8px !important;
        border: none !important;
        box-shadow: 0 4px 10px rgba(30, 58, 138, 0.2) !important;
        font-weight: 600 !important;
        letter-spacing: 0.5px;
