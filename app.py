...
# ZMIANA: Dodano kolumnę typ_wysylki
SHEET_HEADERS = {
    ...
    if st.session_state.rola == 'szef':
        ...
        with t1:
            with st.form("add_form", clear_on_submit=True):
                ...
                # ZMIANA: Wybór rodzaju wysyłki u szefa
                typ_wysylki = st.radio("Sposób dostawy", ["Kurier", "Dostawa własna"], horizontal=True)
                ...
                if st.form_submit_button("PRZEKAZ", type="primary") and nr and co:
                    ...
                    # ZMIANA: Dodano zapis typu wysyłki do bazy
                    zam_data.append({"id": new_id, "nr": nr, "co": co, "termin": termin.strftime("%Y-%m-%d"), "ma_etykiete": "True" if plik else "False", "typ_wysylki": typ_wysylki})
        ...
        with t2:
            for z in zam_data:
                with st.expander(f"Zam: {z['nr']} | {z['termin']}"):
                    # Podgląd u szefa
                    st.write(f"**Dostawa:** {z.get('typ_wysylki', 'Brak')}")
            ...
            for i, z in enumerate(zam_prac):
                with cols[i % 3]:
                    with st.container(border=True):
                        ...
                        # Generowanie odznak daty i typu wysyłki
                        dzisiaj_str = datetime.now().strftime("%Y-%m-%d")
                        t_zlec = str(z.get('termin', '9999-12-31'))
                        
                        if t_zlec < dzisiaj_str: badge_data = f"<div style='background-color: #fee2e2; color: #ef4444; padding: 4px 10px; border-radius: 6px; font-weight: 700; display: inline-block; margin-bottom: 10px; font-size: 0.75rem; border: 1px solid #fca5a5;'>ZALEGŁE: {t_zlec}</div>"
                        elif t_zlec == dzisiaj_str: badge_data = f"<div style='background-color: #fef3c7; color: #d97706; padding: 4px 10px; border-radius: 6px; font-weight: 700; display: inline-block; margin-bottom: 10px; font-size: 0.75rem; border: 1px solid #fde68a;'>NA DZISIAJ</div>"
                        else: badge_data = f"<div style='background-color: #f1f5f9; color: #64748b; padding: 4px 10px; border-radius: 6px; font-weight: 600; display: inline-block; margin-bottom: 10px; font-size: 0.75rem; border: 1px solid #e2e8f0;'>TERMIN: {t_zlec}</div>"
                        
                        # ZMIANA: Etykieta rodzaju dostawy u pracownika
                        typ_w = str(z.get('typ_wysylki', ''))
                        if "Kurier" in typ_w:
                            badge_typ = f"<div style='background-color: #e0f2fe; color: #0369a1; padding: 4px 10px; border-radius: 6px; font-weight: 700; display: inline-block; margin-bottom: 10px; margin-left: 8px; font-size: 0.75rem; border: 1px solid #bae6fd;'>KURIER</div>"
                        elif "własna" in typ_w.lower():
                            badge_typ = f"<div style='background-color: #f3e8ff; color: #7e22ce; padding: 4px 10px; border-radius: 6px; font-weight: 700; display: inline-block; margin-bottom: 10px; margin-left: 8px; font-size: 0.75rem; border: 1px solid #e9d5ff;'>DOSTAWA WŁASNA</div>"
                        else:
                            badge_typ = ""
                        
                        st.markdown(f"<div>{badge_data}{badge_typ}</div>", unsafe_allow_html=True)
                        
                        st.markdown(f"<h2 style='margin-top: 0; margin-bottom: 15px;'>{z['nr']}</h2>", unsafe_allow_html=True)
                        st.write(f"**Spec:** {z['co']}")
                        st.write("<br>", unsafe_allow_html=True)
                        
                        # Mechanizm pobierania Blob
